...
class Transaction(object):
    ...
    def __init__(self,
	 ...
	 size=0.000546,
        ...
        self.size = size
        self.fee= fee
        self.tx_type = tx_type  # новый атрибут
...
class LightTransaction():
    ...
    # Генерация атакующих транзакций (например, 30% от общего количества)
        attack_share = 0.3
        attack_count = int(Psize * attack_share)
        legit_count = Psize - attack_count

    # Атакующие транзакции
        for _ in range(attack_count):
            tx = Transaction()
            tx.id = random.randrange(100000000000)
            tx.sender = 0  # предположим, что атакующий — узел 0
            tx.to = random.choice(p.NODES).id
            tx.size = random.expovariate(1 / p.Tsize)
            tx.fee = 0  # можно задать минимальную комиссию
            tx.timestamp = random.randint(0, p.simTime - 1)
            tx.tx_type = "spam"
            pool.append(tx)
    ...
    def execute_transactions(currentTime):
        print("[DEBUG] CSV будет сохранён в:", os.path.abspath("light_tx_delay_log.csv"))
        transactions = []
        size = 0
        count = 0
        blocksize = p.Bsize
        ...
        while count < len(pool):
            ...
            if blocksize >= tx.size:
                ...
                if hasattr(tx, 'tx_type') and tx.tx_type == 'legit':
                    delay = currentTime - tx.timestamp
                    legit_delays.append(delay)
            count += 1

    # Вывод + лог в файл
        #if legit_delays:
            avg_delay = sum(legit_delays) / len(legit_delays)
            print(f"[{currentTime}] Средняя задержка (легитимные): {avg_delay:.2f} сек, Кол-во: {len(legit_delays)}")

            # Путь к файлу
            filename = "light_tx_delay_log.csv"
            ...
            with open(filename, mode='a', newline='') as csvfile:
                ...
                if not file_exists:
                    writer.writerow(['tick', 'avg_delay', 'legit_tx_count'])
                writer.writerow([currentTime, avg_delay, len(legit_delays)])
